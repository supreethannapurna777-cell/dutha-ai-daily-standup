...
function page(
        ...
        const cards = projects.map((project) => {
                ...
                return `<section class="project">
                        ...
                        <div><h3>Team members</h3><table>${memberRows || '<tr><td>No members configured.</td></tr>'}</table></div></div>
                </section>`;
        }).join("");
        return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Projects and access · Dutha WorkOps</title><style>
:root{font-family:Inter,Arial,sans-serif;color:#172033;background:#f4f7fb}*{box-sizing:border-box}body{margin:0;padding:30px}main{max-width:1250px;margin:auto}header{display:flex;justify-content:space-between;gap:20px;align-items:start;margin-bottom:22px}h1,h2,h3{color:#173f6b}h1{margin:0 0 6px}.muted,small{display:block;color:#64748b}.top{color:#1769aa;font-weight:700}.notice,.error{padding:13px;border-radius:9px;margin:14px 0}.notice{background:#dcfce7;color:#166534}.error{background:#fee2e2;color:#991b1b}.create,.project{background:#fff;border-radius:13px;box-shadow:0 3px 14px #0f172a12;padding:21px;margin-bottom:20px}.create-grid{display:grid;grid-template-columns:1fr 2fr auto;gap:12px;align-items:end}label{display:grid;gap:6px;font-weight:700}input,select{padding:10px;border:1px solid #cbd5e1;border-radius:8px;font:inherit}button{border:0;border-radius:8px;padding:10px 14px;color:#fff;background:#1769aa;font-weight:700;cursor:pointer}.project-head{display:flex;justify-content:space-between}.project-head h2{margin:5px 0}.key,.status{font-size:12px;font-weight:800}.key{color:#1769aa}.status{color:#166534;background:#dcfce7;padding:6px 9px;border-radius:99px}.columns{display:grid;grid-template-columns:1fr 1fr;gap:24px}table{width:100%;border-collapse:collapse}td{padding:9px;border-bottom:1px solid #e5e7eb}.inline{text-align:right}.assign{background:#15803d}.remove{background:#b91c1c;padding:7px 10px}.manager-form{margin-top:18px;border-top:1px solid #e5e7eb;padding-top:18px}.manager-grid{display:grid;grid-template-columns:1.3fr 1.5fr 1fr auto;gap:10px;align-items:end}@media(max-width:800px){body{padding:15px}.columns,.create-grid,.manager-grid{grid-template-columns:1fr}header{flex-direction:column}}
