...
function page(
        ...
        const whatsappReady = readiness.whatsapp.configured;
        const teamsReady = readiness.teams.configured;
        const teamsStatus = readiness.teams.enabled
                ? statusPill(teamsReady, teamsReady ? "Configured" : "Needs configuration")
                : statusPill(false, "Planned — not in this release");
        const jiraReady = readiness.jira.configured && readiness.jira.webhookConfigured;
...
:root{font-family:Inter,Arial,sans-serif;color:#dbeafe;background:#070b18;color-scheme:dark}*{box-sizing:border-box}body{margin:0;padding:28px;min-height:100vh;background:radial-gradient(circle at 6% 0,#1e3a8a 0,transparent 34%),radial-gradient(circle at 94% 4%,#312e81 0,transparent 30%),#070b18}main{max-width:1180px;margin:auto}header{display:flex;justify-content:space-between;gap:20px;align-items:start;margin-bottom:20px}h1,h2{color:#f8fafc}h1{margin:0 0 6px}.muted,small{display:block;color:#94a3b8}.back{color:#93c5fd;font-weight:700}.notice,.error,.code{padding:14px;border-radius:12px;margin:14px 0}.notice{background:#0c4a6e;color:#e0f2fe}.error{background:#450a0a;color:#fecaca}.code{background:#172554;border:1px solid #3b82f6}.code strong{display:block;font-size:22px;letter-spacing:2px;color:#dbeafe;margin:8px 0}.toolbar,.card,.integration{background:#111827d9;border:1px solid #334155;border-radius:18px;box-shadow:0 18px 50px #0005;backdrop-filter:blur(18px);padding:20px;margin-bottom:18px}.toolbar{display:flex;justify-content:space-between;gap:14px;align-items:end}.toolbar form{display:flex;gap:9px;align-items:end}.integrations{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:18px}.integration{min-height:130px}.integration h2{font-size:18px;margin:0 0 8px}.pill{display:inline-block;padding:5px 9px;border-radius:99px;font-size:12px;font-weight:800}.connected{background:#1e3a8a;color:#bfdbfe}.pending{background:#312e81;color:#ddd6fe}select,input{padding:10px;border:1px solid #475569;border-radius:10px;font:inherit;background:#0b1220;color:#f8fafc}input[type=file]{max-width:100%}button{border:0;border-radius:10px;padding:10px 13px;background:linear-gradient(135deg,#2563eb,#7c3aed);color:#fff;font-weight:800;cursor:pointer}.secondary{background:#334155;padding:7px 10px}.danger{background:#991b1b;padding:7px 10px}.create,.import-form{display:flex;gap:12px;align-items:end;flex-wrap:wrap}.create label,.import-form label{display:grid;gap:6px}.email-form,.invite-actions{display:flex;gap:7px}.email-form input{min-width:230px}.inline-action{margin-top:7px}.remove-form{display:grid;gap:7px;margin-top:8px;min-width:180px}.remove-form label{font-size:12px}.remove-form input{display:block;width:100%;margin-top:4px;padding:7px}summary{cursor:pointer;color:#fca5a5;font-weight:700}.import-help{background:#0f172a;border:1px solid #334155;border-radius:10px;padding:11px;margin:10px 0}.import-help code{display:block;margin-top:5px;overflow-wrap:anywhere}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:11px;border-bottom:1px solid #273449;vertical-align:middle}th{color:#93c5fd;font-size:13px}.scroll{overflow:auto}.actions{display:flex;gap:10px;flex-wrap:wrap}.actions a{color:#93c5fd;font-weight:700}@media(max-width:800px){body{padding:14px}header,.toolbar{flex-direction:column}.integrations{grid-template-columns:1fr}.email-form{min-width:260px}table{min-width:980px}}
...
async function render(
        ...
        const [memberRows, inviteRows, readiness] = await Promise.all([
                members(env.DB, actor.tenantId, projectId),
                invites(env.DB, actor.tenantId, projectId),
                integrationReadiness(env),
        ]);
        return response(page(projectId, options, memberRows, inviteRows, env, generatedCode, notice, error, readiness), error ? 400 : 200);
}
